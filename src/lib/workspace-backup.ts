...
import { isSrBreaksSettings } from './sr-breaks-retests'
...
function indicator(value: unknown): Indicator {
  ...
  if (kind === 'sr-breaks-retests' && i.sr !== undefined) {
    const settings = i.sr
    if (!isSrBreaksSettings(settings)) invalid('SR Breaks and Retests settings')
    result.sr = {
      ...
      showVolumeText: settings.showVolumeText,
      showHoldSignals: settings.showHoldSignals,
      showRetestSignals: settings.showRetestSignals,
      showBreakLabels: settings.showBreakLabels,
    }
